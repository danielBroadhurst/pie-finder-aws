...
import { CreatePieStoreService } from './create-pie-store.service';
...
export const createPieStoreUseCase = async function (event: {
  ...
  try {
    const pieStoreRepository: PieStoreRepositoryPort = new PieStoreRepository();
    const commandService = new CreatePieStoreService(pieStoreRepository);
